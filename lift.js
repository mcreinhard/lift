...
(function() {
  var binaryOpFunctions, binaryOpProto, binaryOps, unaryOpFunctions, unaryOpProto, unaryOps,
    __slice = [].slice;
  ...
  unaryOpProto = _.zipObject(unaryOps, _.map(unaryOps, function(op) {
    return eval("(function(a) {return " + op + " a;})");
  }));

  binaryOpProto = _.zipObject(binaryOps, _.map(binaryOps, function(op) {
  ...
  lift.op.register = function(name, f) {
    (f.length === 1 ? unaryOpFunctions : binaryOpFunctions)[name] = f;
    return _.extend(lift.op, _.mapValues(unaryOpFunctions, lift));
  };
